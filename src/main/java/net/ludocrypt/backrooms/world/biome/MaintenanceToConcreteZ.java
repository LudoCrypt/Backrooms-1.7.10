...
public class MaintenanceToConcreteZ extends MaintenanceBaseBiome {

	public MaintenanceToConcreteZ(int i) {
		super(i);
		...
		this.downfall = 0.0F;
		this.setSeedModifier(6727669);
	}

	@Override
	public void generateRoom(World world, long seed, Random random, BlockPos pos) {
		BackroomsRooms.MAINTENANCE_TO_CONCRETE_Z.generate(world, seed, random, pos);
	}

	@Override
	public void cleanupRoom(World world, long seed, Random random, BlockPos pos) {
		BackroomsRooms.MAINTENANCE_TO_CONCRETE_Z.cleanup(world, seed, random, pos);
	}
