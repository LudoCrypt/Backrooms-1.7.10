...
import net.ludocrypt.backrooms.mixin.SimplexNoiseSamplerAccessor;
...
public final class PerlinNoiseSampler {
	...
	public double sample(double x, double y, double z, double d, double e) {
		...
		double m = g - (double) j;
		double n = h - (double) k;
		double o = MathHelperUpdated.perlinFade(l);
		double p = MathHelperUpdated.perlinFade(m);
		double q = MathHelperUpdated.perlinFade(n);
		double t;
		if (d != 0.0D) {
	...
	private static double grad(int hash, double x, double y, double z) {
		int i = hash & 11;
		return MathHelperUpdated.dot(SimplexNoiseSamplerAccessor.getField_2171()[i], x, y, z);
	}
	...
	public double sample(int sectionX, int sectionY, int sectionZ, double localX, double localY, double localZ, double fadeLocalX, double fadeLocalY, double fadeLocalZ) {
		...
		return MathHelperUpdated.lerp3(fadeLocalX, fadeLocalY, fadeLocalZ, d, e, f, g, h, o, p, q);
	}
